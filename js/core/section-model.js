(function (root, factory) {
  ...
  function buildHalfHourPresenceSeries(rows, hours, options = {}) {
    ...
    const presenceMultiplier = typeof options.presenceMultiplier === 'function' ? options.presenceMultiplier : () => 1;
    const counts = {};
    ...
    (rows || []).forEach((row, index) => {
      ...
      const multiplier = metric === 'presence' ? Number(presenceMultiplier(section, row)) : 1;
      const enrollment = metric === 'presence' ? enrollmentForSection(section) * (Number.isFinite(multiplier) ? multiplier : 1) : 1;
