(function () {
  ...
  const state = {
    ...
    optimizationProposedEvaluation: [],
    optimizationBetterTimes: [],
    optimizationAudit: [],
    optimizationContext: {},
    optimizationRows: [],
  ...
  function isAttritionCensus2Label(value) {
    return /census\s*2/i.test(String(value || ''));
  }

  function attritionVisibleItems(items) {
    return SHOW_CENSUS2 ? items : (items || []).filter(item => !isAttritionCensus2Label(item?.label || item?.[0] || item));
  }

  function attritionVisibleColumns(columns) {
    return SHOW_CENSUS2 ? columns : (columns || []).filter(column => !isAttritionCensus2Label(column?.label || column?.key || column));
  }
  ...
  function normalize(row) {
    ...
    const normalized = {
      ...
      sourceFileName: row.__sourceFileName || '',
      sourceUploadedAt: row.__uploadedAt || row.uploadedAt || row.UploadedAt || ''
    };
      ...
      <section id="analyticsReports" class="analytics-reports" style="display:none">
        ...
        <div id="studentPresenceReport" class="analytics-view">
          <div class="analytics-report-intro">
            ...
            <div class="analytics-methodology">
              ...
              <div>
                <h3>Methodology</h3>
                <ul>
                  <li>Nominal Scheduled Presence counts every valid physical meeting at full enrollment. Expected Physical Presence applies a Meeting Frequency Factor so hybrid or irregular meetings contribute proportionally to average campus presence.</li>
          ...
          <div class="analytics-toolbar">
            ...
            <label>Presence Mode
              <select id="spPresenceMode">
                <option value="nominal" selected>Nominal Scheduled Presence</option>
                <option value="expected">Expected Physical Presence</option>
              </select>
            </label>
        ...
        <div id="scheduleOptimizationLabReport" class="analytics-view">
          ...
          <div class="analytics-toolbar">
            ...
            <label>Term preset
              <select id="optimizationTermPreset">
                <option value="">Manual term selection</option>
                <option value="prior-like">Prior like terms</option>
                <option value="prior-fall">Prior Fall Terms</option>
                <option value="prior-spring">Prior Spring Terms</option>
                <option value="prior-academic-year">Prior Academic Year</option>
              </select>
            </label>
            ...
            <button id="exportOptimizationProposed" type="button">Export Proposed Time Evaluation</button>
            <button id="exportOptimizationBetterTimes" type="button">Export Better Times</button>
          ...
          <section id="optimizationAddClassPanel" class="demand-report-section">
            ...
            <div class="analytics-toolbar">
              <label>Subject <select id="optimizationAddSubject"><option value="">All subjects</option></select></label>
              <label>Course <select id="optimizationAddCourseSelect"><option value="">Select course from data</option></select></label>
              <label>Existing section/CRN <select id="optimizationAddSectionProfile"><option value="">Optional profile</option></select></label>
              <label>Manual course <input id="optimizationAddCourse" type="text" placeholder="ENGL C1000"></label>
              ...
              <label>Proposed day pattern <input id="optimizationProposedDays" type="text" value="MW"></label>
              <label>Proposed start <input id="optimizationProposedStart" type="time" value="09:00" step="1800"></label>
              <label>Proposed end <input id="optimizationProposedEnd" type="time" value="10:15" step="1800"></label>
              <label>Proposed campus <input id="optimizationProposedCampus" type="text" placeholder="COS"></label>
              <label>Proposed room type <input id="optimizationProposedRoomType" type="text" value="Classroom"></label>
              <label>Expected enrollment/cap <input id="optimizationProposedEnrollment" type="number" min="1" value="35"></label>
              <label>Faculty note <input id="optimizationFacultyNote" type="text" placeholder="Optional preferred-time note"></label>
              <button id="runOptimizationPlacement" type="button">Rank Placement Options</button>
            </div>
            <div id="optimizationCourseProfileStatus" class="analytics-legend"></div>
            <div id="optimizationProposedEvaluation" class="analytics-table"></div>
            <div id="optimizationBetterTimes" class="analytics-table"></div>
            ...
            <div class="analytics-methodology">
              ...
              <div>
                ...
                <ul>
                  ...
                  <li>Overall Attrition = Census 1 to End/Final attrition. Census 1 is captured from CENSUS_ENROLL. End/Final uses final enrollment or ACTUAL_ENROLL/current enrollment after the end of the term.</li>
    ...
    const batches = await Promise.all(files.map(file => new Promise((resolve, reject) => {
      ...
      Papa.parse(file, {
        ...
        complete: (r) => resolve((r.data || []).map(row => ({
          ...row,
          __sourceTerm: sourceTerm,
          __sourceType: options.sourceType || '',
  ...
  const SUPPLY_DEMAND_PLANNING_WINDOWS = {
    MO: { start: 8 * 60, end: 20 * 60 },
    TU: { start: 8 * 60, end: 20 * 60 },
    WE: { start: 8 * 60, end: 20 * 60 },
    TH: { start: 8 * 60, end: 20 * 60 },
    FR: { start: 8 * 60, end: 15 * 60 }
  };

  function supplyDemandPlanningWindowLabel() {
    return 'Monday-Thursday 8:00 AM-8:00 PM; Friday 8:00 AM-3:00 PM; Saturday/Sunday excluded';
  }

  function supplyDemandPlanningWindowEnabled() {
    const control = document.getElementById('sdPlanningWindow');
    return control ? control.checked !== false : true;
  }

  function supplyDemandPlanningSlots() {
    const slots = [];
    for (let minutes = 8 * 60; minutes < 20 * 60; minutes += 30) slots.push(minutes);
    return slots;
  }

  function isSupplyDemandPlanningInterval(day, minutes) {
    const window = SUPPLY_DEMAND_PLANNING_WINDOWS[day];
    return !!window && minutes >= window.start && minutes < window.end;
  }

  function supplyDemandRowStartsInPlanningWindow(row) {
    const start = minutesFromTime(row?.start || row?.startTime);
    const end = minutesFromTime(row?.end || row?.endTime);
    if (start == null || end == null || end <= start) return false;
    return (row.days || []).some(day => {
      const window = SUPPLY_DEMAND_PLANNING_WINDOWS[day];
      return !!window && start >= window.start && start < window.end;
    });
  }

  function supplyDemandSlots(rows, options = {}) {
    if (options.planningWindow === true) return supplyDemandPlanningSlots();
  ...
  function buildSupplyDemandBuckets(rows, metricName, options = {}) {
    const dayKeys = options.planningWindow === true ? SUPPLY_DEMAND_PLANNING_DAYS : ['SU', 'MO', 'TU', 'WE', 'TH', 'FR', 'SA'];
    const dayNames = { SU: 'Sunday', MO: 'Monday', TU: 'Tuesday', WE: 'Wednesday', TH: 'Thursday', FR: 'Friday', SA: 'Saturday' };
    const fixedRows = physicalIntervalRows(rows, options);
    const planningRows = options.planningWindow === true ? fixedRows.filter(supplyDemandRowStartsInPlanningWindow) : fixedRows;
    const slots = supplyDemandSlots(planningRows, options);
    const map = new Map();
    dayKeys.forEach(day => {
      slots.forEach(minutes => {
        const key = `${day}|${minutes}`;
    ...
    planningRows.forEach(row => {
      ...
      row.days.forEach(day => {
        ...
        slots.forEach(minutes => {
          if (options.planningWindow === true && !isSupplyDemandPlanningInterval(day, minutes)) return;
          if (end <= minutes || start >= minutes + 30) return;
    ...
    return {
      ...
      fixedRows,
      planningRows,
      planningWindow: options.planningWindow === true,
      planningWindowLabel: options.planningWindow === true ? supplyDemandPlanningWindowLabel() : 'All scheduled days/times',
      excludedOutsidePlanningWindow: Math.max(0, fixedRows.length - planningRows.length)
    };
  }
  ...
  function renderSupplyDemandLineGraph(built, metricName) {
    ...
    const dayNames = built.dayKeys.map(day => built.dayNames[day]);
    const width = 920;
    const height = 320;
    const left = 48;
    const right = 16;
  ...
  async function runSupplyDemand() {
    ...
    const intervalOptions = {
      includeOnline: includeOnlineFromSelect('sdModality'),
      planningWindow: supplyDemandPlanningWindowEnabled()
    };
  ...
  function clearSupplyDemand() {
    ...
    const planningWindow = document.getElementById('sdPlanningWindow');
    if (planningWindow) planningWindow.checked = true;
  ...
  function optimizationAllRows() {
    return (state.optimizationRows?.length ? state.optimizationRows : state.enrollment || []).map(normalize);
  }
  ...
  async function loadOptimizationRows() {
    ...
    if (!state.optimizationRows.length && state.enrollment?.length) {
      state.optimizationRows = dedupeEnrollmentRows(state.enrollment.map(normalize));
    }
  ...
  function optimizationSelectedTerms(id) {
    return getSelectedValues(id).map(normalizeTermLabel).filter(Boolean);
  }

  function optimizationLikeHistoricalTerms(activeTerm, terms) {
    const active = termParts(activeTerm);
    return terms.filter(term => {
      const parts = termParts(term);
      return parts.year && parts.year < active.year && (!active.season || parts.season === active.season);
    }).sort((a, b) => termSortValue(b) - termSortValue(a)).slice(0, 5);
  }

  function applyOptimizationTermPreset() {
    const preset = document.getElementById('optimizationTermPreset')?.value || '';
    if (!preset) return;
    const active = document.getElementById('optimizationTerm')?.value || '';
    const terms = [...new Set(optimizationAllRows().map(row => normalizeTermLabel(row.term)).filter(Boolean))]
      .sort((a, b) => termSortValue(a) - termSortValue(b));
    const activeParts = termParts(active);
    let selected = [];
    if (preset === 'prior-like') selected = optimizationLikeHistoricalTerms(active, terms);
    if (preset === 'prior-fall') selected = terms.filter(term => termParts(term).season === 'FALL' && term !== active);
    if (preset === 'prior-spring') selected = terms.filter(term => termParts(term).season === 'SPRING' && term !== active);
    if (preset === 'prior-academic-year') {
      const trailingYear = activeParts.season === 'FALL' ? activeParts.year : activeParts.year - 1;
      selected = terms.filter(term => {
        const parts = termParts(term);
        return parts.year && (parts.year === trailingYear || parts.year === trailingYear - 1) && term !== active;
      });
    }
    ['optimizationHistoryTerms', 'optimizationDemandTerms'].forEach(id => {
      const select = document.getElementById(id);
      if (!select) return;
  ...
  function updateOptimizationTermOptions() {
    ...
    ['optimizationHistoryTerms', 'optimizationDemandTerms'].forEach(id => {
      ...
      if (!multi) return;
      const prior = Array.from(multi.selectedOptions || []).map(option => option.value);
      resetSelect(multi, terms.filter(term => term !== select.value), 'All', 'All');
      const wanted = new Set(prior.filter(term => terms.includes(term) && term !== select.value));
      if (!wanted.size && id === 'optimizationDemandTerms') {
        optimizationLikeHistoricalTerms(select.value, terms).forEach(term => wanted.add(term));
      }
      Array.from(multi.options).forEach(option => { option.selected = wanted.has(option.value); });
    });
    applyOptimizationTermPreset();
  }

  function optimizationActiveRows() {
    const term = document.getElementById('optimizationTerm')?.value || '';
    const rows = optimizationAllRows();
    return term ? rows.filter(row => normalizeTermLabel(row.term) === term) : rows;
  }

  function optimizationHistoryRows(activeTerm) {
    const rows = optimizationAllRows();
    const selectedHistory = optimizationSelectedTerms('optimizationHistoryTerms');
    const selectedDemand = optimizationSelectedTerms('optimizationDemandTerms');
    const selected = [...new Set([...selectedHistory, ...selectedDemand])].filter(Boolean);
    if (selected.length) return rows.filter(row => selected.includes(normalizeTermLabel(row.term)));
    return rows.filter(row => normalizeTermLabel(row.term) !== activeTerm);
  }
  ...
  function optimizationAddClassInput() {
    ...
    const proposedStart = document.getElementById('optimizationProposedStart')?.value || document.getElementById('optimizationAddStart')?.value || '09:00';
    const proposedEnd = document.getElementById('optimizationProposedEnd')?.value || '';
    return {
      subject: document.getElementById('optimizationAddSubject')?.value || '',
      course: manualCourse || selectedCourse,
      selectedCourse,
      sectionProfileKey: document.getElementById('optimizationAddSectionProfile')?.value || '',
      expectedEnrollment: document.getElementById('optimizationAddEnrollment')?.value || document.getElementById('optimizationProposedEnrollment')?.value || '',
      campus: document.getElementById('optimizationAddCampus')?.value || document.getElementById('optimizationProposedCampus')?.value || '',
      modality: document.getElementById('optimizationAddModality')?.value || 'In-Person',
      roomType: document.getElementById('optimizationAddRoomType')?.value || document.getElementById('optimizationProposedRoomType')?.value || '',
      ...
      proposedDayPattern: document.getElementById('optimizationProposedDays')?.value || document.getElementById('optimizationAddDays')?.value || 'MW',
      proposedStart,
      proposedEnd,
      proposedCampus: document.getElementById('optimizationProposedCampus')?.value || document.getElementById('optimizationAddCampus')?.value || '',
      proposedRoomType: document.getElementById('optimizationProposedRoomType')?.value || document.getElementById('optimizationAddRoomType')?.value || '',
      cap: document.getElementById('optimizationProposedEnrollment')?.value || document.getElementById('optimizationAddEnrollment')?.value || '',
      facultyNote: document.getElementById('optimizationFacultyNote')?.value || ''
    };
  }

  function updateOptimizationCourseControls() {
    const engine = optimizationEngine();
    const profiles = engine.courseProfiles(optimizationAllRows());
    state.optimizationCourseProfiles = profiles;
    const subjectSelect = document.getElementById('optimizationAddSubject');
    const courseSelect = document.getElementById('optimizationAddCourseSelect');
    const sectionSelect = document.getElementById('optimizationAddSectionProfile');
    if (!subjectSelect || !courseSelect || !sectionSelect) return;
    const priorSubject = subjectSelect.value;
    const priorCourse = courseSelect.value;
    const priorSection = sectionSelect.value;
    const subjects = [...new Set(profiles.map(profile => profile.subject).filter(Boolean))]
      .sort((a, b) => a.localeCompare(b, undefined, { numeric: true }));
    resetSelect(subjectSelect, subjects, 'All subjects', '');
    subjectSelect.value = subjects.includes(priorSubject) ? priorSubject : '';
    const visibleProfiles = subjectSelect.value ? profiles.filter(profile => profile.subject === subjectSelect.value) : profiles;
    resetSelect(courseSelect, visibleProfiles.map(profile => ({ value: profile.courseCode, label: profile.courseCode })), 'Select course from data', '');
    courseSelect.value = visibleProfiles.some(profile => profile.courseCode === priorCourse) ? priorCourse : '';
    const selectedProfile = profiles.find(profile => profile.courseCode === courseSelect.value);
    const sections = selectedProfile?.sections || [];
    resetSelect(sectionSelect, sections.map((section, index) => ({
      value: `${selectedProfile.courseCode}::${index}`,
      label: [section.term, section.crn ? `CRN ${section.crn}` : '', section.section, section.dayTime].filter(Boolean).join(' - ')
    })), 'Optional profile', '');
    sectionSelect.value = sections.some((_, index) => `${selectedProfile?.courseCode}::${index}` === priorSection) ? priorSection : '';
    document.getElementById('optimizationCourseProfileStatus').innerHTML = profiles.length
      ? `<strong>Course/section profile source</strong><p>${escapeAttr(profiles.length)} course profile(s) available from loaded active and historical schedule rows. Selecting a course or CRN can prefill Add-a-Class assumptions; manual entry remains available for new courses.</p>`
      : '<p class="analytics-empty">Load schedule rows or archived terms to populate course and section dropdowns. Manual entry is still available.</p>';
  }

  function prefillOptimizationAddClassFromProfile() {
    const profiles = state.optimizationCourseProfiles || optimizationEngine().courseProfiles(optimizationAllRows());
    const selectedCourse = document.getElementById('optimizationAddCourseSelect')?.value || '';
    const selectedSectionKey = document.getElementById('optimizationAddSectionProfile')?.value || '';
    const profile = profiles.find(item => item.courseCode === selectedCourse);
    if (!profile) return;
    const sectionIndex = Number((selectedSectionKey.split('::')[1] || ''));
    const section = Number.isFinite(sectionIndex) ? profile.sections[sectionIndex] : null;
    const set = (id, value) => {
      const node = document.getElementById(id);
      if (node && value !== undefined && value !== null && value !== '') node.value = value;
    };
    set('optimizationAddCourse', profile.courseCode);
    set('optimizationAddEnrollment', section?.enrollment || profile.typicalEnrollment);
    set('optimizationProposedEnrollment', section?.cap || profile.historicalCap || profile.typicalEnrollment);
    set('optimizationAddCampus', section?.campus || profile.campus);
    set('optimizationProposedCampus', section?.campus || profile.campus);
    set('optimizationAddModality', section?.modality || profile.modality);
    set('optimizationAddRoomType', section?.roomType || profile.typicalRoomType);
    set('optimizationProposedRoomType', section?.roomType || profile.typicalRoomType);
    set('optimizationAddDivision', section?.division || profile.division);
  }
  ...
  async function runScheduleOptimizationLab() {
    await renderScheduleOptimizationLab();
  }

  async function runOptimizationPlacement() {
    await renderScheduleOptimizationLab();
  }
  ...
  function exportOptimizationRows(kind) {
    const map = {
      ...
      proposed: [state.optimizationProposedEvaluation, 'schedule-optimization-proposed-time-evaluation.csv'],
      betterTimes: [state.optimizationBetterTimes, 'schedule-optimization-better-time-recommendations.csv'],
  ...
  function studentPresenceFilteredSectionRows() {
    ...
    const rows = rowsForTerm
      ...
      .filter(item => {
        const rowStart = minutesFromTime(item.section.start);
        const rowEnd = minutesFromTime(item.section.end);
        return item.section.days?.includes(filter.dayCode) && rowStart != null && rowEnd != null && rowStart < end && rowEnd > start;
      });
    const seen = new Set();
    return rows.reduce((items, item) => {
  ...
  function renderStudentPresenceCurve(rows) {
    ...
    const options = studentPresenceOptions();
    const expectedPresence = options.presenceMode === 'expected';
    ...
    node.innerHTML = `
      <section class="presence-curve">
        ...
        <p>Student Presence estimates how many enrolled students are scheduled to be physically present during each half-hour interval. Nominal Scheduled Presence counts each distinct CRN/day/start/end block at full enrollment. Expected Physical Presence applies a Meeting Frequency Factor when hybrid or irregular meetings occur for only part of the term. Duplicate rows for the same CRN/day/start/end count once; the same CRN with a different day or different start/end counts as a distinct meeting block.</p>
  ...
  function demandExecutiveSummary(rows, trends, context = {}, diagnostics = {}) {
    ...
    const drivers = [
      expected ? `Historical Trend Expected Enrollment variance is ${projectedVariance >= 0 ? '+' : ''}${Math.round(projectedVariance)} students against the historical benchmark projection.` : 'Trend projection is unavailable for this scope.',
    ...
    return {
      ...
      capDelta,
      diagnostics,
      warnings: context.projectionWarnings || [],
      currentFtes,
      projectedFtes,
      ftesVariance,
  ...
  function renderDemandMetricGroups(config = {}) {
    ...
    const activeSnapshot = config.activeSnapshot || {};
    const confidenceDetails = config.confidenceDetails || summary.confidence || {};
    ...
    const metricRows = [
      ...
      ['Executive Summary', [
        ['Historical Trend Expected Enrollment', Math.round(summary.projected || collegeRow.expectedEnrollmentNextTerm || 0), 'projected-final-enrollment'],
        ['Current Snapshot Enrollment', Math.round(activeSnapshot.currentEnrollment || summary.current || 0), 'current-enrollment'],
        ['Enrollment Difference from Current Snapshot', Math.round((summary.projected || collegeRow.expectedEnrollmentNextTerm || 0) - (activeSnapshot.currentEnrollment || summary.current || 0)), 'projected-variance'],
        ['Historical Trend Estimated FTES', round1(config.forecastFtes || 0), 'ftes'],
        ['Current Snapshot Estimated FTES', activeSnapshot.estimatedFtes ? round1(activeSnapshot.estimatedFtes) : 'N/A', 'ftes'],
  ...
  function demandReportContextOverrides(config = {}) {
    ...
    const activeSnapshot = config.activeSnapshot || {};
    const diagnostics = config.termDiagnostics || {};
    const loadedRows = config.loadedRows || state.demandInput || [];
    const sourceFiles = [...new Set([
      ...(activeSnapshot.sourceFiles || []),
      ...loadedRows.map(row => row.sourceFileName || row.raw?.__sourceFileName || '').filter(Boolean)
    ])];
    return {
      prefix: 'dem',
      focusTerm: config.target?.label || demandForecastTarget().label,
      historicalTerms: diagnostics.termsUsedInForecast || [],
      archivedTerms: diagnostics.selectedArchivedTerms || getSelectedValues('demArchiveTerms'),
      rowsLoaded: loadedRows.length,
      rowsIncluded: (config.rows || []).length,
      rowsExcluded: Math.max(0, loadedRows.length - (config.rows || []).length),
      ...
      method: [
        ...
        { label: 'Current snapshot source files', value: sourceFiles.length ? sourceFiles.join(', ') : 'Source file unavailable' },
        { label: 'As-of date / snapshot date', value: activeSnapshot.snapshotDate || (activeSnapshot.hasRows ? 'Upload date unavailable' : 'No active snapshot loaded') },
        { label: 'Active term completeness', value: activeSnapshot.isIncompleteActiveTerm ? 'Incomplete active term - current enrollment used because census is unavailable' : 'No active-term incompleteness flag' },
        { label: 'Historical terms used', value: termListLabel(diagnostics.termsUsedInForecast || []) },
        { label: 'Comparison terms used', value: termListLabel(diagnostics.termsIncludedAfterFilters || []) },
        { label: 'Historical term count', value: (diagnostics.termsUsedInForecast || []).length },
        { label: 'Enrollment basis', value: 'Census preferred, actual/current fallback' },
        { label: 'Scheduled Class Offerings', value: 'Unique CRNs' },
        { label: 'Dual Enrollment in totals', value: selections.dual ? 'Included as subset of total' : 'Excluded from totals' },
        { label: 'Tutoring/Open Lab excluded', value: document.getElementById('demExcludeTutoringOpenLab')?.checked ? 'Yes' : 'No' },
        { label: 'FTES methodology', value: 'Historical Trend Estimated FTES = Historical Trend Expected Enrollment x Historical FTES per Enrollment Ratio; planning estimate, not reportable FTES' },
        { label: 'FTES composition', value: 'Instructional + Dual Enrollment + Work Experience = Total; Dual Enrollment is a subset/breakout, not additive beyond total.' },
        { label: 'Conservative-High FTES Range', value: 'Uses the enrollment confidence/range spread applied through the historical FTES per enrollment ratio.' }
      ],
      notes: [
  ...
  async function runDemand() {
    ...
    try {
      ...
      const activeSnapshot = demandActiveSnapshot(filtered, target);
  ...
  function renderEmptyDemand(message, termDiagnostics = null, context = {}) {
    ...
    metric('demandMetrics', [
      ...
      ['Courses Reviewed', 0],
      ['Historical FTES', 0],
      ['Historical Trend Estimated FTES', 0],
  ...
  function demandHistoricalBenchmarkProjection(termRows = [], currentTotals = {}, options = {}) {
    ...
    const conservativeFtes = historicalFtesPerEnrollment > 0 ? conservative * historicalFtesPerEnrollment : baselineFtes * (1 - rangeSpread);
    let highFtes = historicalFtesPerEnrollment > 0 ? high * historicalFtesPerEnrollment : baselineFtes * (1 + rangeSpread);
    const ftesWarnings = [];
    ...
    const projectedFtesPerEnrollment = safeDiv(mostLikelyFtes, mostLikely);
    if (historicalFtesPerEnrollment > 0 && projectedFtesPerEnrollment > 0 && Math.abs(safeDiv(projectedFtesPerEnrollment - historicalFtesPerEnrollment, historicalFtesPerEnrollment)) > 0.1) {
      ftesWarnings.push('Projected FTES per enrollment differs from the historical FTES per enrollment ratio by more than 10%.');
    }
    const ftesDiagnostics = {
      historicalEnrollment: baselineEnrollment,
      historicalFtes: baselineFtes,
      historicalFtesPerEnrollment,
      currentSnapshotEnrollment: currentEnrollment,
      currentSnapshotFtes: currentFtes,
      currentFtesPerEnrollment,
      projectedEnrollment: mostLikely,
      projectedFtes: mostLikelyFtes,
      projectedFtesPerEnrollment,
      rangeSpread,
      warnings: ftesWarnings
    };
    return {
      ...trend,
      ...
      expectedFtesRange: {
        conservative: conservativeFtes,
        low: conservativeFtes,
        mostLikely: mostLikelyFtes,
        high: highFtes
      },
      ftesProjectionMethod: 'Historical FTES per Enrollment Ratio',
      ftesProjectionFormula: 'Historical Trend Estimated FTES = Historical Trend Expected Enrollment x Historical FTES per Enrollment Ratio',
      ftesPerEnrollmentDiagnostics: ftesDiagnostics,
      ftesProjectionWarnings: ftesWarnings,
      audit: trend.audit ? {
        ...trend.audit,
  ...
  function demandForecastRow(level, groupName, rows, context, modifierGrowth = 0) {
    ...
    return {
      ...
      expectedFtesRangeMostLikely: expectedFtesRange.mostLikely ?? expectedFtesNextTerm,
      ...
      historicalFtesPerEnrollment: trendModel?.ftesPerEnrollmentDiagnostics?.historicalFtesPerEnrollment ?? safeDiv(avgFtes, avgCensusEnrollment),
      currentFtesPerEnrollment: trendModel?.ftesPerEnrollmentDiagnostics?.currentFtesPerEnrollment ?? safeDiv(currentTotals.ftes, currentTotals.enrollment),
      ...
      avgAttritionRate,
      avgWaitlistCount,
      projectedWaitlistLabel: hasWaitlistData ? 'Estimated Projected Waitlist' : 'Projected Waitlist unavailable',
      projectedWaitlistMethod: hasWaitlistData ? 'Historical average waitlist from comparable completed terms' : 'No waitlist source columns were loaded; waitlist is not estimated',
  ...
  function demandTargetTerms(target = {}) {
    if (target.scope === 'year') {
      return ['SUMMER', 'FALL', 'SPRING'].map(season => normalizeTermLabel(targetTermFromFiscalYear(season, target.year))).filter(Boolean);
    }
    return [normalizeTermLabel(target.label || target.term)].filter(Boolean);
  }

  function demandActiveSnapshot(rows = [], target = {}) {
    const terms = new Set(demandTargetTerms(target));
    const activeRows = (rows || []).filter(row => terms.has(normalizeTermLabel(row.sourceTerm || row.term)));
    const distinctCrns = demandDistinctCrnCount(activeRows);
    ...
    const sourceFiles = [...new Set(activeRows.map(row => row.sourceFileName || row.raw?.__sourceFileName || '').filter(Boolean))].sort();
    const explicitSnapshotDate = snapshotDates.at(-1) || '';
    const uploadDate = uploadDates.at(-1) || '';
    const snapshotDate = explicitSnapshotDate || (uploadDate ? `Uploaded ${uploadDate}` : (activeRows.length ? 'Upload date unavailable' : ''));
    return {
      termLabel: target.label || target.term || '',
      rows: activeRows.length,
      distinctCrns,
      sectionCount: distinctCrns,
      currentEnrollment,
      censusEnrollment,
      seatsOffered,
      fillRate: safeDiv(censusEnrollment || currentEnrollment, seatsOffered),
      estimatedFtes,
      snapshotDate,
      explicitSnapshotDate,
      uploadDate,
      sourceFiles,
  ...
  function demandProjectionWarnings(summary = {}, college = {}, snapshot = {}, threshold = 0.1) {
    const warnings = [];
    const projectedEnrollment = Number(summary.projected || college.expectedEnrollmentNextTerm || 0);
    const snapshotEnrollment = Number(snapshot.currentEnrollment || 0);
    if (snapshotEnrollment > 0 && projectedEnrollment > 0) {
      const diff = safeDiv(projectedEnrollment - snapshotEnrollment, snapshotEnrollment);
      if (Math.abs(diff) > threshold) {
        warnings.push(`Historical Trend Expected Enrollment is ${pct(Math.abs(diff))} ${diff >= 0 ? 'higher' : 'lower'} than the current snapshot. This is a historical trend expectation, not actual current enrollment.`);
      }
    }
    const projectedFtes = Number(college.expectedFtesNextTerm || 0);
    const currentFtes = Number(snapshot.estimatedFtes || 0);
    if (currentFtes > 0 && projectedFtes > 0) {
  ...
  function demandConfidenceDetails(rows = [], trends = [], context = {}, diagnostics = {}, snapshot = {}) {
    const base = demandForecastConfidenceScore(rows);
    const terms = diagnostics.termsUsedInForecast || [];
    const lowered = [];
    const raised = [];
    const college = rows.find(row => row.forecastLevel === 'College') || {};
    const volatility = average((trends || []).map(row => Math.abs(Number(row.fillRate || 0) - average((trends || []).map(item => Number(item.fillRate || 0))))));
    if (terms.length >= 3) raised.push(`${terms.length} like-term historical terms are listed and used.`);
    ...
    if (snapshot.explicitSnapshotDate) raised.push('Current snapshot/as-of date is available.');
    else if (snapshot.uploadDate) lowered.push('No explicit snapshot date was provided; upload timestamp is used as the as-of proxy.');
    else lowered.push('Current snapshot date is missing or unavailable.');
    if (snapshot.isIncompleteActiveTerm) lowered.push('Active/current term appears incomplete because current enrollment is present but census enrollment is unavailable.');
    ...
    const ftesDiagnostics = college.ftesPerEnrollmentDiagnostics || {};
    if ((college.ftesProjectionWarnings || []).length) (college.ftesProjectionWarnings || []).forEach(item => lowered.push(item));
    if (Number(ftesDiagnostics.historicalFtesPerEnrollment || 0) > 0 && Number(ftesDiagnostics.currentFtesPerEnrollment || 0) > 0) {
      const ratioDiff = Math.abs(safeDiv(Number(ftesDiagnostics.currentFtesPerEnrollment) - Number(ftesDiagnostics.historicalFtesPerEnrollment), Number(ftesDiagnostics.historicalFtesPerEnrollment)));
      if (ratioDiff > 0.1) lowered.push('Current FTES/enrollment ratio diverges by more than 10% from the historical ratio.');
    }
    if (college.scheduleAdjustment?.combinedFactor && Math.abs(Number(college.scheduleAdjustment.combinedFactor) - 1) <= 0.1) raised.push('Current schedule size is within 10% of historical schedule baseline.');
    const score = Math.max(20, Math.min(95, base.score + raised.length * 3 - lowered.length * 5));
    const label = score >= 80 && lowered.length <= 2 ? 'High' : score >= 55 ? 'Medium' : 'Low';
    return {
      label,
      ...
      improve: [
        ...
        'Load an explicit snapshot/as-of date with current active-term files.',
        'Compare current FTES-per-enrollment against historical ratios before treating FTES as a target.',
  ...
  function renderDemandInsights(rows, patterns, trends, yearSeasonForecast = null, forecastContext = {}) {
    ...
    wrap.innerHTML = `
      <section class="demand-report-section demand-executive-summary" data-collapsible-title="Executive Summary" data-collapsible-id="demand-executive-summary" data-collapsible-default-open="true">
        ...
        ${demandForecastTypePanel(forecastContext)}
        ${demandProjectionWarningsPanel(forecastContext.projectionWarnings || summary.warnings || [])}
        ...
        ${demandCurrentSnapshotPanel(forecastContext.activeSnapshot || {})}
        ${demandFtesDiagnosticsPanel(collegeRow)}
      ...
      <section class="demand-report-section" data-collapsible-title="Data Quality & Methodology" data-collapsible-id="demand-diagnostics-methodology" data-collapsible-default-open="false">
        ...
        ${demandConfidencePanel(forecastContext.confidenceDetails || summary.confidence || {})}
        ${demandProjectionCalculationPanel(forecastContext, rows, trends, summary, collegeRow)}
  ...
  function buildEnrollmentTrendChartData(trends, forecastEnrollment = null, targetLabel = 'Forecast') {
    ...
    return {
      ...
      series: [
        ...
        { name: 'Historical Trend Expected Enrollment', values: forecast }
      ],
      tooltips: categories.map((term, index) => {
        ...
        return analyticsTooltip([
          ...
          ['Historical Trend Expected Enrollment', forecast[index] ?? 'N/A'],
  ...
  function buildFtesTrendChartData(trends, forecastFtes = 0, ftesCap = 0, targetLabel = 'Forecast') {
    ...
    return {
      ...
      series: [
        ...
        { name: 'Historical Trend Estimated FTES', values: forecast },
  ...
  function demandExecutiveNarrative(summary = {}, context = {}) {
    ...
    return `<p class="demand-executive-narrative">Based on ${terms || 'selected'} comparable historical term${terms === 1 ? '' : 's'}, recency-weighted growth, and current schedule construction, ${escapeAttr(target)} has a Historical Trend Expected Enrollment of approximately ${projected} students, ${variancePct} versus the historical trend baseline, with ${escapeAttr(demandConfidenceLabel(summary.confidence?.label))} confidence. This is not actual current enrollment.</p>`;
  }
  ...
  function demandExecutiveRecommendation(summary = {}, findings = [], rows = [], trends = [], context = {}) {
    ...
    const primaryReason = capOver
      ...
      : variance
        ? `Historical Trend Expected Enrollment is ${variance >= 0 ? '+' : ''}${Math.round(variance)} students versus the expected benchmark.`
        : 'Historical Trend Expected Enrollment is close to the historical benchmark.';
    const actions = [
    ...
    const evidence = [
      `Historical Trend Expected Enrollment: ${Math.round(projected)} students.`,
  ...
  function demandCompositionPanel(summary = {}, title = 'FTES Composition', mode = 'ftes') {
    ...
    return `<section class="demand-composition-panel"><h4>${escapeAttr(title)}</h4><p class="analytics-chart-note">Dual Enrollment may be shown as a breakout. Unless otherwise stated, it is a subset of total enrollment/FTES, not an additional amount.</p>${rows}<div class="demand-composition-total"><span>Total including selected populations</span><strong>${mode === 'enrollment' ? Math.round(total) : round1(total)}</strong><em>100%</em></div></section>`;
  }
  ...
  function demandFtesWaterfallPanel(context = {}, populationSummary = {}) {
    ...
    const rows = [
      ...
      ['Instructional FTES subset', instructional],
      ['Dual Enrollment subset', dual],
      ['Work Experience subset', work],
      ['Historical Trend Estimated FTES total', projected]
    ];
  ...
  function demandPlanningExportRows(config = {}) {
    ...
    const confidence = context.confidenceDetails || config.summary?.confidence || {};
    const college = (config.calculationRows || []).find(row => row.forecastLevel === 'College') || {};
    const rows = [];
    rows.push(
    ...
    [
      ...
      ['Current Snapshot Section Count', activeSnapshot.sectionCount || 0],
      ['Current Snapshot Seats Offered', activeSnapshot.seatsOffered || 0],
      ['Current Snapshot Fill Rate', pct(activeSnapshot.fillRate || 0)],
      ['Current Snapshot Estimated FTES', activeSnapshot.estimatedFtes ? round1(activeSnapshot.estimatedFtes) : 'N/A'],
      ['Snapshot Date / As-of Date', activeSnapshot.snapshotDate || (activeSnapshot.hasRows ? 'Upload date unavailable' : 'No active snapshot loaded')],
      ['Current Snapshot Source Files', (activeSnapshot.sourceFiles || []).join(', ') || 'Source file unavailable'],
      ['Active Term Completeness', activeSnapshot.isIncompleteActiveTerm ? 'Incomplete active term - census unavailable' : 'No active-term incompleteness flag'],
      ['Historical Trend Expected Enrollment', Math.round(college.expectedEnrollmentNextTerm || config.summary?.projected || 0)],
      ['Conservative Enrollment Estimate', Math.round(college.expectedRangeConservative ?? college.expectedRangeLow ?? 0)],
      ['Most Likely Estimate', Math.round(college.expectedRangeMostLikely ?? college.expectedEnrollmentNextTerm ?? 0)],
      ['High Enrollment Estimate', Math.round(college.expectedRangeHigh ?? 0)],
      ['Historical Trend Estimated FTES', round1(college.expectedFtesNextTerm || context.forecastFtes || 0)],
      ['Conservative FTES Estimate', round1(college.expectedFtesRangeLow || 0)],
      ...
      ['Current FTES per Enrollment Ratio', round1(Number(college.currentFtesPerEnrollment || 0) * 1000) / 1000],
    ...
    (confidence.raised || []).forEach((item, index) => rows.push({ Section: 'Confidence Factors - Increased', Field: `Factor ${index + 1}`, Value: item }));
    (confidence.lowered || []).forEach((item, index) => rows.push({ Section: 'Confidence Factors - Lowered', Field: `Factor ${index + 1}`, Value: item }));
    (context.projectionWarnings || []).forEach((item, index) => rows.push({ Section: 'Projection Warnings', Field: `Warning ${index + 1}`, Value: item }));
    (college.ftesProjectionWarnings || []).forEach((item, index) => rows.push({ Section: 'FTES Projection Warnings', Field: `Warning ${index + 1}`, Value: item }));
    [
  ...
  function demandProjectionWarningsPanel(warnings = []) {
    if (!warnings.length) return '';
    return `<section class="demand-health-card demand-health-watch"><div><span>Projection Warnings</span><strong>${warnings.length}</strong><p>Review before treating the trend projection as a planning target.</p></div><ul>${warnings.map(item => `<li>${escapeAttr(item)}</li>`).join('')}</ul></section>`;
  }

  function demandCurrentSnapshotPanel(snapshot = {}) {
    return `<section class="demand-method-card">
      <h4>Current Snapshot Baseline</h4>
      <p>These values come only from rows matching the active/forecast target term when such rows are loaded. They are shown separately from the historical trend projection.</p>
      <dl>
        <div><dt>Current Snapshot Enrollment</dt><dd>${Math.round(snapshot.currentEnrollment || 0)}</dd></div>
        <div><dt>Current Snapshot Section Count</dt><dd>${snapshot.sectionCount || 0}</dd></div>
        <div><dt>Current Snapshot Seats Offered</dt><dd>${snapshot.seatsOffered || 0}</dd></div>
        <div><dt>Current Snapshot Fill Rate</dt><dd>${pct(snapshot.fillRate || 0)}</dd></div>
        <div><dt>Current Snapshot Estimated FTES</dt><dd>${snapshot.estimatedFtes ? round1(snapshot.estimatedFtes) : 'N/A'}</dd></div>
        <div><dt>Snapshot Date / As-of Date</dt><dd>${escapeAttr(snapshot.snapshotDate || (snapshot.hasRows ? 'Upload date unavailable' : 'No active snapshot loaded'))}</dd></div>
        <div><dt>Source Files</dt><dd>${escapeAttr((snapshot.sourceFiles || []).join(', ') || 'Source file unavailable')}</dd></div>
        <div><dt>Active Term Completeness</dt><dd>${snapshot.isIncompleteActiveTerm ? 'Incomplete active term - census unavailable' : 'No active-term incompleteness flag'}</dd></div>
      </dl>
    </section>`;
  }

  function demandFtesDiagnosticsPanel(row = {}) {
    const diagnostics = row.ftesPerEnrollmentDiagnostics || {};
    const warnings = row.ftesProjectionWarnings || diagnostics.warnings || [];
    return `<section class="demand-method-card demand-ftes-diagnostics">
      <h4>FTES per Enrollment Diagnostics</h4>
      <p>Historical Trend Estimated FTES is calculated from the projected enrollment using the historical FTES per enrollment ratio. Current snapshot FTES is shown separately when available.</p>
      <dl>
        <div><dt>Historical Enrollment</dt><dd>${Math.round(diagnostics.historicalEnrollment || row.historicalBaselineEnrollment || 0)}</dd></div>
        <div><dt>Historical FTES</dt><dd>${round1(diagnostics.historicalFtes || row.avgFtes || 0)}</dd></div>
        <div><dt>Historical FTES / Enrollment</dt><dd>${round1(Number(diagnostics.historicalFtesPerEnrollment || row.historicalFtesPerEnrollment || 0) * 1000) / 1000}</dd></div>
        <div><dt>Current Snapshot Enrollment</dt><dd>${Math.round(diagnostics.currentSnapshotEnrollment || 0)}</dd></div>
        <div><dt>Current Snapshot FTES</dt><dd>${round1(diagnostics.currentSnapshotFtes || 0)}</dd></div>
        <div><dt>Current FTES / Enrollment</dt><dd>${round1(Number(diagnostics.currentFtesPerEnrollment || 0) * 1000) / 1000}</dd></div>
  ...
  function demandConfidencePanel(details = {}) {
    return `<section class="demand-method-card">
      ...
      <div class="demand-action-grid">
        ...
        <div><h4>How to Improve Confidence</h4><ul>${(details.improve || []).map(item => `<li>${escapeAttr(item)}</li>`).join('')}</ul></div>
      </div>
    </section>`;
  }

  function demandProjectionCalculationPanel(context = {}, rows = [], trends = [], summary = {}, college = {}) {
    ...
    return `<section class="demand-method-card" data-collapsible-title="How This Projection Was Calculated" data-collapsible-id="demand-projection-calculation-path" data-collapsible-default-open="false">
      ...
      <dl>
        ...
        <div><dt>Conservative Enrollment Estimate</dt><dd>${Math.round(college.expectedRangeConservative ?? college.expectedRangeLow ?? 0)}</dd></div>
        <div><dt>Most likely estimate</dt><dd>${Math.round(college.expectedRangeMostLikely ?? college.expectedEnrollmentNextTerm ?? 0)}</dd></div>
        <div><dt>High Enrollment Estimate</dt><dd>${Math.round(college.expectedRangeHigh ?? 0)}</dd></div>
        <div><dt>Conservative FTES Estimate</dt><dd>${round1(college.expectedFtesRangeLow || 0)}</dd></div>
        <div><dt>Most Likely FTES Estimate</dt><dd>${round1(college.expectedFtesRangeMostLikely || college.expectedFtesNextTerm || 0)}</dd></div>
        <div><dt>High FTES Estimate</dt><dd>${round1(college.expectedFtesRangeHigh || 0)}</dd></div>
        <div><dt>FTES formula</dt><dd>${escapeAttr(college.ftesProjectionFormula || 'Historical Trend Estimated FTES = Historical Trend Expected Enrollment x Historical FTES per Enrollment Ratio')}</dd></div>
  ...
  function demandForecastMethodCard(context = {}, summary = {}, backtest = {}) {
    ...
    return `<section class="demand-method-card">
      ...
      <dl>
        ...
        <div><dt>FTES caution</dt><dd>Historical Trend Estimated FTES is ratio-based planning context and should not be treated as reportable FTES.</dd></div>
  ...
  function renderAttritionLegend() {
    ...
    const items = attritionVisibleItems([
      ...
      ['Total Milestone Enrollment', SHOW_CENSUS2 ? 'Historical Census 1, Census 2, and End/Final totals sum all valid values for each milestone independently. They do not become zero or N/A simply because the CRN populations differ.' : 'Historical Census 1 and End/Final totals sum all valid values for each milestone independently. They do not become zero or N/A simply because the CRN populations differ.'],
      ['Matched CRNs', SHOW_CENSUS2 ? 'For each attrition interval, only CRNs with both required milestone values are included. Example: Census 2 to End uses only CRNs with valid Census 2 and valid End/Final values.' : 'For each attrition interval, only CRNs with both required milestone values are included.'],
      ...
      ['Lifecycle Readiness', SHOW_CENSUS2 ? 'First Day comes from stored First Day snapshots when available. Census 1 comes from CENSUS_ENROLL. Census 2 comes from CENSUS_ENROLL2. End/Final comes from ACTUAL_ENROLL or Final Enrollment. Missing milestone fields display as N/A, not zero.' : 'First Day comes from stored First Day snapshots when available. Census 1 comes from CENSUS_ENROLL. End/Final comes from ACTUAL_ENROLL or Final Enrollment. Missing milestone fields display as N/A, not zero.'],
      ...
      ['Diagnostic Attrition Rates', SHOW_CENSUS2 ? 'Separate table showing historical trend rates for First Day to Census 1, First Day to Census 2, First Day to End/Final, Census 1 to Census 2, Census 1 to End/Final, and Census 2 to End/Final.' : 'Separate table showing historical trend rates for First Day to Census 1, First Day to End/Final, and Census 1 to End/Final.'],
  ...
  function renderDemandLegend() {
    ...
    const items = [
      ...
      ['Historical Trend Expected Enrollment', 'Enrollment expected from historical same-season or academic-year trends after recency weighting and schedule adjustment. This is separate from current snapshot enrollment. Expected Projection Range shows conservative, most likely, and high enrollment estimates. Conservative-High FTES Range applies the enrollment range spread through the historical FTES per enrollment ratio.'],
      ...
      ['Historical Trend Estimated FTES', 'Metric card. Estimated FTES from the historical trend projection. Formula: Historical FTES x (1 + adjusted forecast growth). This is a planning estimate, not reportable FTES.'],
      ...
      ['Annual FTES Projection', 'Metric card. For single-term forecasts, Historical Trend Estimated FTES + Known/Projected Companion FTES. For academic-year forecasts, this equals Historical Trend Estimated FTES. This is the value used for the FTES cap comparison.'],
  ...
  function label(text) {
    const labels = {
      ...
      firstDayToCensus1AttritionDisplay: 'First Day to Census 1 Attrition',
      firstDayToEndAttritionDisplay: 'First Day to End/Final Attrition',
      ...
      projectedFtes: 'Historical Trend Estimated FTES',
      ...
      projectedEnrollment: 'Historical Trend Expected Enrollment',
      ...
      expectedRangeConservative: 'Conservative Enrollment Estimate',
      expectedRangeMostLikely: 'Most Likely Estimate',
      expectedFtesRangeDisplay: 'Conservative-High FTES Range',
      expectedFtesRangeLow: 'Conservative FTES Estimate',
      expectedFtesRangeMostLikely: 'Most Likely FTES Estimate',
      expectedFtesRangeHigh: 'High FTES Estimate',
      historicalFtesPerEnrollment: 'Historical FTES per Enrollment Ratio',
      currentFtesPerEnrollment: 'Current FTES per Enrollment Ratio',
      projectedFtesPerEnrollment: 'Projected FTES per Enrollment Ratio',
      ftesProjectionMethod: 'FTES Projection Method',
      ftesProjectionFormula: 'FTES Projection Formula',
      ftesProjectionWarningSummary: 'FTES Projection Warnings',
      projectedWaitlistLabel: 'Projected Waitlist Label',
      projectedWaitlistMethod: 'Projected Waitlist Method',
  ...
  function format(value, column = '') {
    ...
    if (typeof value === 'number' && /attrition/i.test(column)) return pct(value);
  ...
  window.COSEnrollmentAnalytics = {
    ...
    supplyDemandSlots,
    supplyDemandPlanningWindowLabel,
    isSupplyDemandPlanningInterval,
    supplyDemandRowStartsInPlanningWindow,
