document.addEventListener('DOMContentLoaded', () => {
  ...
  let currentData=[], currentTerm='', tsDiv, roomDiv, uploadDiv, table, container;

  // Cache DOM refs after structure loads
  function initRefs() {
    tsDiv=document.getElementById('upload-timestamp');
    roomDiv=document.getElementById('room-filter');
    uploadDiv=document.getElementById('upload-container');
  ...
  // Populate availability dropdowns
  const startSel=document.getElementById('avail-start'), endSel=document.getElementById('avail-end');
  for(let m=360;m<=22*60;m+=5){
    const h=Math.floor(m/60), mm=m%60, ap=h<12?'AM':'PM',h12=(h+11)%12+1,label=`${h12}:${('0'+mm).slice(-2)} ${ap}`;
  ...
  function selectTerm(term,tabElem){
    ...
    if(currentTerm){
      ...
      localStorage.setItem(key,JSON.stringify({data:currentData,timestamp:tsDiv.textContent}));
    }
    currentTerm=term;
    // highlight
    ...
    if(saved){
      ...
      currentData=obj.data; tsDiv.textContent=obj.timestamp;
      buildRoomDropdown(); renderSchedule();
    } else {
      currentData=[]; tsDiv.textContent=''; roomDiv.innerHTML='';
    }
  }
  ...
  function clearSchedule(){
    table.innerHTML=''; container.querySelectorAll('.class-block').forEach(e=>e.remove());
    ...
    for(let t=360;t<=22*60;t+=30){
      const row=table.insertRow(), hh=Math.floor(t/60),mm=t%60,h12=(hh+11)%12+1,ap=hh<12?'AM':'PM';
  ...
  function renderSchedule(){
    ...
    daysOfWeek.forEach((day,dIdx)=>{
      ...
      cols.flat().forEach(ev=>{
        const minutesFromStart=ev.startMin-360,remainder=minutesFromStart%30;
        const rowIndex=Math.floor(minutesFromStart/30)+1;
        const cell=table.rows[rowIndex].cells[dIdx+1],cellRect=cell.getBoundingClientRect();
        ...
        block.style.top=topPx+'px';block.style.left=leftPx+'px';
        block.style.width=widthPx+'px';block.style.height=heightPx+'px';
  ...
  function handleAvailability(){
    ...
    if(!days.length||!s||!e){resultsDiv.textContent='Select days and times.';return;}
    const toMin=t=>{const [time,ap]=t.split(' ');let [h,m]=time.split(':').map(Number);if(ap==='PM'&&h<12)h+=12;if(ap==='AM'&&h===12)h=0;return h*60+m;};
    const sMin=toMin(s),eMin=toMin(e),rooms=[...new Set(currentData.map(i=>i.Building+'-'+i.Room))],occ=new Set();
    currentData.forEach(i=>{if(i.Days.some(d=>days.includes(d))){const si=parseTime(i.Start_Time),ei=parseTime(i.End_Time);if(!(ei<=sMin||si>=eMin))occ.add(i.Building+'-'+i.Room);}});
    const avail=rooms.filter(r=>!occ.has(r));
    if(avail.length) resultsDiv.innerHTML='<ul>'+avail.map(r=>`<li>${r}</li>`).join('')+'</ul>';
