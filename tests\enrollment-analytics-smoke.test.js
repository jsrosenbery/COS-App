...
test('growth opportunities mark lower confidence when waitlist data is unavailable', () => {
  const { COSEnrollmentDashboard } = loadEnrollmentModules();
  const rows = [
    section({ subject: 'MATH', course: '021', crn: 'M1', census: 30, cap: 30, waitlist: 0, hasWaitlistData: false }),
    section({ subject: 'MATH', course: '021', crn: 'M2', census: 18, cap: 20, waitlist: 0, hasWaitlistData: false })
  ];

  const [opportunity] = COSEnrollmentDashboard.growthOpportunities(rows);

  assert.equal(opportunity.hasWaitlistData, false);
  assert.match(opportunity.confidence, /Lower confidence/);
});
...
test('modality balance exposes DE toggle and comparison controls', () => {
  const html = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');
  const app = fs.readFileSync(path.join(__dirname, '..', 'js/app.js'), 'utf8');

  assert.match(html, /id="modality-include-de"/);
  assert.match(html, /id="modality-decision-term"/);
  assert.match(html, /id="modality-comparison-1"/);
  assert.match(app, /isDualEnrollmentMethod/);
  assert.match(app, /renderModalityComparison/);
});

test('duration graph exposes readable concurrent-course axis helper', () => {
  const app = fs.readFileSync(path.join(__dirname, '..', 'js/app.js'), 'utf8');

  assert.match(app, /function concurrentCourseAxis/);
  assert.match(app, /niceSteps = \[1, 2, 5, 10, 20, 25, 50, 100\]/);
  assert.match(app, /window\.COSLineChart/);
});

test('calendar renders in standalone section outside schedule table', () => {
  const html = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');

  assert.match(html, /<section id="calendar-container"/);
  assert.doesNotMatch(html, /<td[^>]*>\s*<div id="calendar-container"/);
});

test('student presence has independent archive and upload controls', () => {
  const source = fs.readFileSync(path.join(__dirname, '..', 'js/enrollment-analytics.js'), 'utf8');

  assert.match(source, /id="studentPresenceCsv"/);
  assert.match(source, /id="spArchiveTerms"/);
  assert.match(source, /readArchivedRows\('spArchiveTerms'\)/);
  assert.match(source, /Shared dashboard data/);
});

test('instructor availability exposes division discipline scope controls', () => {
  const source = fs.readFileSync(path.join(__dirname, '..', 'js/enrollment-analytics.js'), 'utf8');

  assert.match(source, /id="iaDivision"/);
  assert.match(source, /id="iaDiscipline"/);
  assert.match(source, /id="iaSelectVisible"/);
  assert.match(source, /Scoped Visible Instructors/);
});

test('consolidation supports future-term historical planning candidates', () => {
  const source = fs.readFileSync(path.join(__dirname, '..', 'js/enrollment-analytics.js'), 'utf8');

  assert.match(source, /id="conDecisionSeason"/);
  assert.match(source, /historicalPlanningCandidates/);
  assert.match(source, /Historical Planning Candidate/);
  assert.match(source, /Review future schedule build for possible section count adjustment/);
});
