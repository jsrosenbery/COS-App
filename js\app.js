...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const modalityComparisonSelects = [
    document.getElementById('modality-comparison-1'),
    document.getElementById('modality-comparison-2'),
    document.getElementById('modality-comparison-3')
  ];
  ...
  modalityComparisonSelects.forEach(select => {
    if (select) select.addEventListener('change', renderModalityTool);
  });
...
document.getElementById('export-pdf-btn').addEventListener('click', function() {
  ...
  async function fetchScheduleRows(term) {
    if (scheduleCache.has(term)) return scheduleCache.get(term);
    const res = await fetch(`${BACKEND_BASE_URL}/api/schedule/${encodeURIComponent(term)}`);
    const payload = await res.json();
    const rows = (payload.data || []).map(normalizeRow);
    scheduleCache.set(term, rows);
    return rows;
  }
  ...
  function loadScheduleFromBackend(term) {
    fetch(`${BACKEND_BASE_URL}/api/schedule/${encodeURIComponent(term)}`)
      ...
      .then(({ data, lastUpdated }) => {
        ...
        scheduleCache.set(term, currentData);
  ...
  function initModalityTermControls() {
    const termOptions = terms.map(term => ({ label: term, value: term }));
    if (modalityDecisionTermSelect) {
      const prior = modalityDecisionTermSelect.value || currentTerm;
      resetSelect(modalityDecisionTermSelect, termOptions, 'Active term', '');
      modalityDecisionTermSelect.value = terms.includes(prior) ? prior : currentTerm;
    }
    modalityComparisonSelects.forEach(select => {
      if (!select) return;
      const prior = select.value;
      resetSelect(select, termOptions, 'None', '');
      if (terms.includes(prior)) select.value = prior;
    });
  }

  function isDualEnrollmentMethod(method) {
    const code = normalizeModalityCode(method);
    const text = String(method || '').toLowerCase();
    return code === 'DE' || /dual enrollment|\bdual\b/.test(text);
  }

  function enrollmentForSection(section) {
    const value = extractField(section, [
      'Census_Enroll', 'CENSUS_ENROLL', 'Census Enroll', 'Census Enrollment',
      'Actual_Enroll', 'ACTUAL_ENROLL', 'Actual Enroll', 'Enrollment', 'Enroll', 'ENROLLED', 'Current Enrollment'
    ]);
    const parsed = Number(String(value || '').replace(/[%,$]/g, '').trim());
    return Number.isFinite(parsed) ? parsed : 0;
  }

  function calculateModalityBalance(dataRows = currentData) {
    ...
    const includeDe = Boolean(modalityIncludeDe?.checked);
    const seenSections = new Set();
    const categories = new Map();

    dataRows.forEach((section, index) => {
      ...
      const isDe = isDualEnrollmentMethod(rawMethod);
      if (!includeDe && isDe) return;
      if (omittedModalityCodes.has(normalizeModalityCode(rawMethod)) && !(includeDe && isDe)) return;
      const category = isDe ? 'Dual Enrollment' : getModalityCategory(rawMethod);
      ...
      bucket.count += 1;
      bucket.enrollment += enrollmentForSection(section);
    ...
    const enrollmentTotal = Array.from(categories.values()).reduce((sum, item) => sum + item.enrollment, 0);
    const order = ['In Person', 'Online', 'Hybrid', 'Dual Enrollment', 'Flex', 'Other', 'Unspecified'];
    return Array.from(categories.values())
      .map(item => ({
        ...item,
        share: total ? item.count / total : 0,
        enrollmentShare: enrollmentTotal ? item.enrollment / enrollmentTotal : 0,
        methodDetails: Array.from(item.methods.entries())
          .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))
      }))
      .sort((a, b) => order.indexOf(a.category) - order.indexOf(b.category))
      .map(item => ({ ...item, total, enrollmentTotal }));
  }

  async function renderModalityTool() {
    if (!modalitySummary || !modalityChart || !modalityTable) return;
    const decisionTerm = modalityDecisionTermSelect?.value || currentTerm;
    const comparisonTerms = modalityComparisonSelects.map(select => select?.value || '').filter(Boolean);
    const termRows = await Promise.all([decisionTerm, ...comparisonTerms].filter(Boolean).map(async term => ({
      term,
      rows: calculateModalityBalance(await fetchScheduleRows(term))
    })));
    const rows = termRows.find(item => item.term === decisionTerm)?.rows || calculateModalityBalance();
    const total = rows[0]?.total || 0;
    const enrollmentTotal = rows[0]?.enrollmentTotal || 0;
    ...
    renderModalityComparison(termRows, decisionTerm);
  ...
  function renderModalityComparison(termRows, decisionTerm) {
    if (!modalityComparison || !termRows.length) return;
    const categories = ['In Person', 'Online', 'Hybrid', 'Dual Enrollment', 'Other', 'Unspecified'];
    const decisionRows = termRows.find(item => item.term === decisionTerm)?.rows || [];
    const decisionByCategory = new Map(decisionRows.map(row => [row.category, row]));
    const htmlRows = termRows.flatMap(({ term, rows }) => {
      const byCategory = new Map(rows.map(row => [row.category, row]));
      return categories.map(category => {
        const row = byCategory.get(category) || { count: 0, enrollment: 0, share: 0 };
        const decisionShare = decisionByCategory.get(category)?.share || 0;
        const variance = term === decisionTerm ? '' : `${((row.share - decisionShare) * 100).toFixed(1)} pts`;
        return `<tr><td>${escapeHTML(term)}</td><td>${escapeHTML(category)}</td><td>${row.count}</td><td>${row.enrollment || 0}</td><td>${(row.share * 100).toFixed(1)}%</td><td>${variance}</td></tr>`;
      });
    }).join('');
    modalityComparison.innerHTML = `
      <table class="modality-table">
        <thead><tr><th>Term</th><th>Category</th><th>Sections</th><th>Enrollment</th><th>Share</th><th>Variance vs Decision</th></tr></thead>
  ...
  function renderLineChart() {
    ...
    const axis = concurrentCourseAxis(maxY);
  ...
  function concurrentCourseAxis(maxY) {
    const max = Math.max(1, Math.ceil(Number(maxY) || 0));
    const niceSteps = [1, 2, 5, 10, 20, 25, 50, 100];
    let stepSize = niceSteps.find(step => Math.ceil(max / step) <= 8) || 100;
    if (max <= 10) stepSize = 1;
    const yMax = Math.max(stepSize, Math.ceil(max / stepSize) * stepSize);
    const tickCount = Math.min(10, Math.max(3, Math.floor(yMax / stepSize) + 1));
    return { stepSize, yMax, tickCount };
  }

  window.COSLineChart = {
    ...(window.COSLineChart || {}),
    concurrentCourseAxis
  };
