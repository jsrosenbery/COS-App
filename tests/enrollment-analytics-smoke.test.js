...
test('enrollment analytics report labels are operational', () => {
  ...
  assert.match(text, /Section Consolidation/);
...
test('current enrollment FTES calculation labels reflect standardized method without renaming Banner code', () => {
  const { COSEnrollmentAnalytics } = loadEnrollmentAnalyticsRuntime();
  const row = COSEnrollmentAnalytics.normalizeRow({ term: 'FALL 2026', CRN: 'STD2', Subject: 'MATH', Course: '101', 'ACCOUNTING METHOD': 'W', ACTUAL_ENROLL: 20, CENSUS_ENROLL: 20, SESSION_CREDIT_HOURS: 3, LECTURE_UNITS: 3 });
...
test('FTES hardening fixtures preserve provenance enrollment basis and unavailable classifications', () => {
  ...
  assert.equal(COSEnrollmentAnalytics.currentEnrollmentFtesValue(rows.positiveScheduledOnly), 0);
  assert.equal(Number(COSEnrollmentAnalytics.currentEnrollmentFtesValue(rows.positiveActual).toFixed(6)), Number((382.05 / 525).toFixed(6)));
...
test('demand active snapshot excludes scheduled-hours-only P E rows from projected FTES', () => {
  const { COSEnrollmentAnalytics } = loadEnrollmentAnalyticsRuntime();
  const rows = [
    section({
      term: 'FALL 2026',
  ...
  assert.equal(snapshot.ftesClassificationBasis.unavailableSections, 1);
  assert.equal(snapshot.ftesClassificationBasis.estimatedSections, 0);
  assert.equal(snapshot.estimatedFtes, 0);
  assert.equal(snapshot.estimatedFtes, snapshot.ftesClassificationBasis.ftes);
  assert.match(snapshot.ftesMethodology, /unavailable/i);
});
...
test('archive inspection exposes parsed archived schedule validation', () => {
  ...
  assert.match(text, /Archived Schedule/);
...
test('data validation and mapping report is an Admin diagnostic', () => {
  ...
  assert.match(app, /\[REPORTS\.instructionalMethodValidation\]: 'Data Validation'/);
  ...
  assert.match(app, /<h2>Data Validation<\/h2>/);
...
test('current enrollment and FTES marks scheduled-hours-only P E unavailable when prediction is unavailable', () => {
  const { COSEnrollmentAnalytics } = loadEnrollmentAnalyticsRuntime();
  const rows = [
    section({
      term: 'FALL 2026',
  ...
  assert.equal(summary.ftesClassification.focus.unavailableSectionCount, 1);
  assert.equal(summary.ftesClassification.focus.estimatedSections, 0);
  assert.equal(summary.ftesClassification.focus.estimatedFtesTotal, 0);
  assert.equal(detail.ftesClassification, 'Unavailable');
  assert.equal(detail.derivation, 'unavailable');
  assert.match(detail.reason, /historical basis|historical prediction/i);
  assert.equal(exportDetail['FTES Classification'], 'Unavailable');
  assert.equal(exportDetail['Projected Final FTES'], '');
});
...
test('current enrollment FTES excludes zero-enrollment sections from both terms across report outputs', () => {
  const { COSEnrollmentAnalytics } = loadEnrollmentAnalyticsRuntime();
  const rows = [
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2026', CRN: 'KEEP26', Subject: 'ENGL', Course: '001', ACCOUNTING_METHOD: 'W', ACTUAL_ENROLL: '12', CENSUS_ENROLL: '10', CENSUS_ENRL_DATE: '09/10/2026', FTES: '1' }),
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2026', CRN: 'ZERO26', Subject: 'LA', Course: '425', ACCOUNTING_METHOD: 'E', ACTUAL_ENROLL: '0', CENSUS_ENROLL: '0' }),
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2025', CRN: 'KEEP25', Subject: 'ENGL', Course: '001', ACCOUNTING_METHOD: 'W', ACTUAL_ENROLL: '15', CENSUS_ENROLL: '14', FTES: '1.4' }),
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2025', CRN: 'ZERO25', Subject: 'MATH', Course: '400', ACCOUNTING_METHOD: 'W', ACTUAL_ENROLL: '8', CENSUS_ENROLL: '0', FTES: '0' })
  ];
  const summary = COSEnrollmentAnalytics.buildCurrentEnrollmentFtesSummary(rows, {
    focusTerm: 'FALL 2026',
    comparisonTerm: 'FALL 2025',
    effectiveAsOfDate: '2026-08-27'
  });
  const exported = COSEnrollmentAnalytics.currentEnrollmentFtesExportRows(summary);

  assert.equal(summary.focus.classOfferings, 1);
  assert.equal(summary.comparison.classOfferings, 1);
  assert.equal(summary.context.selectedZeroEnrollmentRowsExcluded, 1);
  assert.equal(summary.context.comparisonZeroEnrollmentRowsExcluded, 1);
  assert.equal(summary.rows.map(row => row.crn).join('|'), 'KEEP26');
  assert.equal(summary.ftesClassification.focus.predictedSections, 0);
  assert.equal(summary.breakdowns.ftesComponents.reduce((sum, row) => sum + row.classOfferings, 0), 1);
  assert.equal(exported.some(row => row.Section === 'Detail' && ['ZERO26', 'ZERO25'].includes(row.CRN)), false);
  assert.ok(exported.some(row => row.Section === 'Summary' && row.Metric === 'Selected Zero-Enrollment Sections Excluded' && row.Value === 1));
...
test('current enrollment and FTES classifies CRN-level maturity from census and source as-of dates', () => {
  ...
  assert.equal(byCrn['10004'].ftesMaturityStatus, 'FTES Unavailable');
  ...
  assert.equal(Math.round(summary.maturity.focus.estimatedFtes * 10) / 10, 4);
  ...
  assert.equal(Math.round(summary.maturity.focus.projectedFtes * 10) / 10, 7.2);
...
test('current enrollment FTES classification totals and exports reconcile without official prediction labels', () => {
  ...
  const rows = [
    section({ crn: 'W001', accountingMethod: 'W', hasFtesData: true, hasDirectFtesData: false, weeklyHours: 3, standardizedHours: 54, actual: 30, census: 30, censusEnrollmentDate: '2026-09-10' }),
    section({ crn: 'D001', accountingMethod: 'D', hasFtesData: true, hasDirectFtesData: false, totalContactHours: 52.5, standardizedHours: 52.5, actual: 20, census: 20, censusEnrollmentDate: '2026-08-20' }),
...
test('current enrollment and FTES applies CRN-level authoritative census maturity rules', () => {
  const { COSEnrollmentAnalytics } = loadEnrollmentAnalyticsRuntime();
  const rows = [
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2026', CRN: '40001', Subject: 'ENGL', Course: 'C1000', ACTUAL_ENROLL: '50', CENSUS_ENROLL: '40', MAX_ENROLL: '55', HOURS_PER_WEEK: '3', STANDARDIZED_HOURS: '54', ACCOUNTING_METHOD: 'W', CENSUS_ENRL_DATE: '09/10/2026', PTRM: 'F1', 'START DATE': '08/17/2026', 'END DATE': '12/11/2026', ACTIVITY_DATE: '09/01/2026' }),
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2026', CRN: '40002', Subject: 'HIST', Course: 'C1000', ACTUAL_ENROLL: '30', CENSUS_ENROLL: '25', MAX_ENROLL: '35', HOURS_PER_WEEK: '3', STANDARDIZED_HOURS: '54', ACCOUNTING_METHOD: 'W', CENSUS_ENRL_DATE: '10/15/2026', PTRM: 'L1', 'START DATE': '10/01/2026', 'END DATE': '12/11/2026', ACTIVITY_DATE: '09/01/2026' }),
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2026', CRN: '40003', Subject: 'MATH', Course: 'C1000', ACTUAL_ENROLL: '20', CENSUS_ENROLL: '18', MAX_ENROLL: '25', HOURS_PER_WEEK: '3', STANDARDIZED_HOURS: '54', ACCOUNTING_METHOD: 'IW', CENSUS_ENRL_DATE: '09/10/2026' }),
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2026', CRN: '40004', Subject: 'BIOL', Course: 'C1000', ACTUAL_ENROLL: '16', CENSUS_ENROLL: '14', MAX_ENROLL: '20', TOTAL_CONTACT_HOURS: '54', STANDARDIZED_HOURS: '54', ACCOUNTING_METHOD: 'D', CENSUS_ENRL_DATE: '09/10/2026' }),
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2026', CRN: '40005', Subject: 'CHEM', Course: 'C1000', ACTUAL_ENROLL: '12', CENSUS_ENROLL: '10', MAX_ENROLL: '15', TOTAL_CONTACT_HOURS: '54', STANDARDIZED_HOURS: '54', ACCOUNTING_METHOD: 'ID', CENSUS_ENRL_DATE: '10/15/2026' }),
    ...
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2026', CRN: '40008', Subject: 'WKEX', Course: '020', ACTUAL_ENROLL: '9', MAX_ENROLL: '15', HOURS_PER_WEEK: '3', STANDARDIZED_HOURS: '54', ACCOUNTING_METHOD: 'W', INSTRUCTIONAL_METHOD_CODE: '20', CENSUS_ENRL_DATE: '09/10/2026' }),
  ...
  assert.equal(byCrn['40002'].ftes, (30 * 54) / 525);
  ...
  assert.equal(byCrn['40007'].ftesMaturityStatus, 'FTES Unavailable');
  assert.equal(byCrn['40008'].ftesMaturityStatus, 'FTES Unavailable');
  assert.match(byCrn['40008'].ftesMaturityReason, /FTES unavailable/i);
...
test('FTES reconciliation builds variance root-cause diagnostics without changing formulas', () => {
  const { COSEnrollmentAnalytics } = loadEnrollmentAnalyticsRuntime();
  const cubeRows = [
    { Campus: 'COS', Division: 'Arts', Subject: 'MATH', Course: '001', CRN: '30001', 'Accounting Method': 'W', 'Part of Term': '1', Enrollment: '30', 'Student Contact Hours': '', 'Individual FTES': '100' },
    { Campus: 'COS', Division: 'Arts', Subject: 'MATH', Course: '002', CRN: '30002', 'Accounting Method': 'W', 'Part of Term': '1', Enrollment: '20', 'Student Contact Hours': '', 'Individual FTES': '80' },
    { Campus: 'COS', Division: 'Arts', Subject: 'ENGL', Course: '001', CRN: '30003', 'Accounting Method': 'IW', 'Part of Term': '1', Enrollment: '25', 'Student Contact Hours': '', 'Individual FTES': '90' },
    { Campus: 'COS', Division: 'Career', Subject: 'FIRE', Course: '001', CRN: '30004', 'Accounting Method': 'E', 'Part of Term': 'OE', Enrollment: '15', 'Student Contact Hours': '300', 'Individual FTES': '40' },
    { Campus: 'COS', Division: 'Career', Subject: 'AUTO', Course: '001', CRN: '30005', 'Accounting Method': 'P', 'Part of Term': 'OE', Enrollment: '10', 'Student Contact Hours': '500', 'Individual FTES': '50' },
    { Campus: 'COS', Division: 'Health', Subject: 'NURS', Course: '001', CRN: '30006', 'Accounting Method': 'W', 'Part of Term': '1', Enrollment: '5', 'Student Contact Hours': '', 'Individual FTES': '60' },
    { Campus: 'COS', Division: 'Review', Subject: 'TEST', Course: '001', CRN: '30007', 'Accounting Method': 'ID', 'Part of Term': '1', Enrollment: '5', 'Student Contact Hours': '', 'Individual FTES': '10' },
    { Campus: 'Total by COLUMNS', Division: '', Subject: '', Course: '', CRN: '', 'Accounting Method': '', 'Part of Term': '', Enrollment: '', 'Student Contact Hours': '', 'Individual FTES': '430' }
  ];
  const timberRows = [
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2025', CRN: '30001', Subject: 'MATH', Course: '001', ACTUAL_ENROLL: '28', FTES: '90', ACCOUNTING_METHOD: 'W', Campus: 'COS' }),
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2025', CRN: '30002', Subject: 'MATH', Course: '002', ACTUAL_ENROLL: '20', FTES: '110', ACCOUNTING_METHOD: 'E', Campus: 'COS' }),
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2025', CRN: '30003', Subject: 'ENGL', Course: '001', ACTUAL_ENROLL: '25', FTES: '70', ACCOUNTING_METHOD: 'IW', Campus: 'COS' }),
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2025', CRN: '30004', Subject: 'FIRE', Course: '001', ACTUAL_ENROLL: '15', FTES: '95', ACCOUNTING_METHOD: 'E', Campus: 'COS' }),
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2025', CRN: '30005', Subject: 'AUTO', Course: '001', ACTUAL_ENROLL: '10', ACCOUNTING_METHOD: 'P', Campus: 'COS' }),
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2025', CRN: '30007', Subject: 'TEST', Course: '001', ACTUAL_ENROLL: '5', FTES: '8', ACCOUNTING_METHOD: 'ID', Campus: 'COS' }),
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2025', CRN: '39999', Subject: 'ART', Course: '001', ACTUAL_ENROLL: '5', FTES: '12', ACCOUNTING_METHOD: 'W', Campus: 'COS' })
  ];

  const summary = COSEnrollmentAnalytics.buildFtesReconciliation(cubeRows, timberRows, { term: 'FALL 2025', expectedInstitutionalTotal: 430 });
  const analysis = summary.rootCauseAnalysis;
  const causes = Object.fromEntries(analysis.rootCauseSummary.map(row => [row.name, row]));
  assert.equal(summary.institutionalFtes, 430);
  assert.equal(summary.timberFtes, 385);
  assert.equal(analysis.accountingMethodCrossTab.some(row => row.institutionalMethod === 'W' && row.timberMethod === 'E' && row.crns === 1), true);
  ...
  assert.equal(analysis.materialRows.find(row => row.crn === '30007').rootCause, 'FORMULA MISMATCH');
  const unknownAnalysis = COSEnrollmentAnalytics.buildFtesRootCauseAnalysis([{ crn: '77777', status: 'Variance', variance: 2, absVariance: 2, institutionalFtes: 5, timberFtes: 7 }]);
  assert.equal(unknownAnalysis.materialRows[0].rootCause, 'UNKNOWN');
  assert.equal(Math.round((Object.values(causes).reduce((total, row) => total + row.variance, 0)) * 100000) / 100000, Math.round(analysis.explainedVariance * 100000) / 100000);
  assert.equal(Math.round((analysis.totalVariance - analysis.explainedVariance - analysis.remainingUnexplainedVariance) * 100000) / 100000, 0);
  const exportRows = COSEnrollmentAnalytics.ftesReconciliationExportRows(summary);
  assert.ok(exportRows.some(row => row.Section === 'By CRN' && row.crn === '30002' && row.institutionalAccountingMethod === 'W' && row.timberAccountingMethod === 'E' && row.rootCause === 'ACCOUNTING METHOD MISMATCH'));
  assert.ok(exportRows.some(row => row.Section === 'Variance Root Cause Summary'));
});

test('FTES formula diagnostics simulate supported fixes without mutating production FTES', () => {
  const { COSEnrollmentAnalytics } = loadEnrollmentAnalyticsRuntime();
  const cubeRows = [
    { Campus: 'COS', Division: 'Arts', Subject: 'MATH', Course: '101', CRN: '41001', 'Accounting Method': 'W', 'Part of Term': '1', Enrollment: '20', 'Student Contact Hours': '', 'Individual FTES': '2' },
    { Campus: 'COS', Division: 'Career', Subject: 'WKEX', Course: '020', CRN: '41002', 'Accounting Method': 'D', 'Part of Term': '1', Enrollment: '10', 'Student Contact Hours': '', 'Individual FTES': '10' },
    { Campus: 'COS', Division: 'Career', Subject: 'AUTO', Course: '120', CRN: '41003', 'Accounting Method': 'P', 'Part of Term': 'OE', Enrollment: '10', 'Student Contact Hours': '262.5', 'Individual FTES': '5' },
    { Campus: 'Total by COLUMNS', Division: '', Subject: '', Course: '', CRN: '', 'Accounting Method': '', 'Part of Term': '', Enrollment: '', 'Student Contact Hours': '', 'Individual FTES': '17' }
  ];
  const timberRows = [
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2025', CRN: '41001', Subject: 'MATH', Course: '101', ACTUAL_ENROLL: '20', HOURS_PER_WEEK: '2', ACCOUNTING_METHOD: 'W', Campus: 'COS', Days: 'MW', STARTTIME: '0900', ENDTIME: '1000' }),
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2025', CRN: '41001', Subject: 'MATH', Course: '101', ACTUAL_ENROLL: '20', HOURS_PER_WEEK: '1', ACCOUNTING_METHOD: 'W', Campus: 'COS', Days: 'F', STARTTIME: '1000', ENDTIME: '1100' }),
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2025', CRN: '41002', Subject: 'WKEX', Course: '020', ACTUAL_ENROLL: '10', FTES: '10', ACCOUNTING_METHOD: 'D', Campus: 'COS' }),
    COSEnrollmentAnalytics.normalizeRow({ Term: 'FALL 2025', CRN: '41003', Subject: 'AUTO', Course: '120', ACTUAL_ENROLL: '10', ACCOUNTING_METHOD: 'P', Campus: 'COS' })
  ];
...
test('W and IW production FTES uses component aggregation while protected special methods remain authoritative or unavailable', () => {
  const { COSEnrollmentAnalytics } = loadEnrollmentAnalyticsRuntime();
  const cubeRows = [
  ...
  assert.equal(byCrn.get('51005').timberFtes, null);
  assert.equal(byCrn.get('51005').status, 'Missing From TIMBER');
...
test('D and ID retain the established maximum CRN contact-hour basis when component identity is ambiguous', () => {
  ...
  const rows = [
    normalize({ TOTAL_CONTACT_HOURS: '56.1', SCHD_CODE_SSRMEET: 'LEC', Days: 'M', STARTTIME: '0900', ENDTIME: '1200' }),
    normalize({ TOTAL_CONTACT_HOURS: '56.1', SCHD_CODE_SSRMEET: 'LEC', Days: 'W', STARTTIME: '0900', ENDTIME: '1200' }),
    normalize({ TOTAL_CONTACT_HOURS: '56.1', SCHD_CODE_SSRMEET: 'LAB', Days: 'F', STARTTIME: '0900', ENDTIME: '1200' })
  ];
  const meetings = rows.flatMap(row => row._meetingRows || []);
...
test('requested analytics regression coverage is represented in smoke tests', () => {
  ...
  [
    ...
    /normalizeTermLabel\(row\.term\) === normalizedFocus/,
