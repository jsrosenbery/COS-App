(function () {
  ...
  function growthOpportunities(rows, options = {}) {
    ...
    return [...byCourse.entries()].map(([course, courseRows]) => {
      ...
      const hasWaitlistData = courseRows.some(row => Boolean(row?.hasWaitlistData));
      ...
      return {
        ...
        waitlist,
        hasWaitlistData,
        confidence: hasWaitlistData ? 'Standard' : 'Lower confidence - waitlist unavailable',
        openSeats,
        viableOpenSeats,
        sameModalitySeats,
        onlineSeats,
  ...
  function dashboardSummaryExportRows(summary, context = {}) {
    ...
    (summary?.growth || []).slice(0, 12).forEach(row => {
      ...
      add('Growth Opportunities', 'Recommendation', row.course, row.recommendation, `Viable seats: ${row.viableOpenSeats}; Waitlist: ${row.waitlist}`, `${notes}; Confidence: ${row.confidence || 'Standard'}`);
